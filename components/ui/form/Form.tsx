...
const Form = <T extends { [x: string]: unknown }>({
  onSubmit,
  onError,
  children,
  className,
      ...
      <form
        onSubmit={methods.handleSubmit(
          (allData, event) =>
            onSubmit(
              allData,
              { setError: methods.setError, reset: methods.reset },
              event
            ),
          onError
        )}
        className={twMerge(
          "grid grid-cols-12 gap-x-0 sm:gap-x-4 gap-y-4 sm:gap-y-3",
          className
        )}
      >
...
const NotificationError = ({ message }: { message?: string }) => {
  return message ? (
    <div className="flex flex-row items-center justify-start w-full gap-2 p-2">
      ...
      <p className={twMerge("text-[0.7em] text-red-600 tracking-wide")}>
        {message}
      </p>
    </div>
  ) : (
