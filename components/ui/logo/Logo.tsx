...
const Logo = ({ alt, mode, size, ...props }: LogoProps) => {
    return <>
        <Button className={twMerge('flex flex-row items-center justify-center gap-2')} {...props}>
            <Image className='w-10 xxs:w-14' src={`https://res.cloudinary.com/dvxn9nvjs/image/upload/v1680447716/portfolio/logo/${mode == 'dark' ? 'color_double_mode_dark_ya6enh' : 'color_double_mode_light_ryyabn'}.svg`} alt={alt} width={size} height={size}/>
        </Button>
    </>
}
